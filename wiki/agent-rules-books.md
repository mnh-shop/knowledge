...
This repository contains compressed, decision-equivalent rule files that can be attached to AI agents (Claude Code, OpenAI Codex CLI, Cursor) to constrain their behavior and improve code quality. Each rule set is inspired by (not a substitute for) the original books and ships in three versions optimized for different context budgets. The project includes a formal compression process (`_rule-workbench/PROCESS.md`), conflict-aware loading guidance (`docs/COMPATIBILITY.md`), constructive criticism documentation (`docs/CRITICISM.md`), editor-specific delivery instructions (`docs/USAGE.md`), and a full release matrix with metrics.
...
- **Conflict-aware loading**: `docs/COMPATIBILITY.md` plus per-book subdirectories in `docs/compatibility/` document which rule sets are compatible or conflicting when loaded together; recommends one primary always-on set with on-demand loading for others
