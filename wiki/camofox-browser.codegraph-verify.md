...
**Claim-6: camofox-browser includes a plugin system with 29 lifecycle events across 8 categories, mutating hooks for browser/session creation, and support for custom metrics via prom-client.**

- **Evidence:** `AGENTS.md` "Plugin System" section documents the full plugin API: `register(app, ctx)` receives 27+ context properties including `ctx.sessions`, `ctx.events`, `ctx.auth`, `ctx.ensureBrowser`, `ctx.getSession`, `ctx.withUserLimit`. The events table at `AGENTS.md:325` ("### Events (29)") lists 29 events across 8 categories — Browser Lifecycle (5, `AGENTS.md:329-336`), Session Lifecycle (4, `AGENTS.md:338-343`), Tab Lifecycle (5, `AGENTS.md:346-352`), Content (4, `AGENTS.md:355-361`), Input (4, `AGENTS.md:363-369`), Downloads (2, `AGENTS.md:371-375`), Cookies/Auth (2, `AGENTS.md:377-381`), Server (3, `AGENTS.md:383-388`). Note: 28 events are emitted by core, 1 (`session:storage:export`) by plugins. Mutating hooks (`browser:launching`, `session:creating`, `session:created`, `session:destroyed`) use `events.emitAsync()` so listeners can modify launch options in-place before proceeding. Custom metrics are created via `ctx.createMetric()` which returns no-op stubs when Prometheus is disabled. The YouTube plugin (`plugins/youtube/`) serves as a reference implementation.
- **Source path:** `sources/camofox-browser/AGENTS.md`

**Claim-7: Current version is 1.13.0, deployed via Fly.io fly-replay headers and railway.toml (no fly.toml in the repo).**

- **Evidence:** `package.json:3` declares `"version": "1.13.0"`. For deployment: `server.js:158` installs `fly.replayMiddleware(log)` to route tab requests to the owning machine via the `fly-replay` header, and `server.js:2717,2722` + `server.js:2817` set the `fly-replay` response header (`res.set('fly-replay', 'app=...')`). Helpers live in `lib/fly.js` (imported at `server.js:11`). No `fly.toml` exists anywhere in the repo — only `railway.toml` (deploy via `Dockerfile.ci`, maps Railway's `PORT` to `CAMOFOX_PORT`). The default port 9377 is set in `lib/config.js:88`.
- **Source path:** `sources/camofox-browser/package.json`, `sources/camofox-browser/server.js`, `sources/camofox-browser/lib/fly.js`, `sources/camofox-browser/railway.toml`

**Claim-8: Additional unlisted lib modules handle browser error classification, process management, page-creation recovery, and process ownership; crash telemetry flows through a Cloudflare Worker.**

- **Evidence:** `lib/browser-errors.js` exports `safePageUrl`/`urlDomain` for error classification; `lib/browser-processes.js` matches browser process cmdlines (`camoufox-bin|firefox-esr|firefox|GeckoChildProcess`) for kill logic; `lib/new-page-recovery.js` provides `createPageWithSessionRecovery` for dead-context/timeout recovery; `lib/process-ownership.js` reads `/proc/<pid>/status` + `/stat` to detect browser/Xvfb ownership. Crash telemetry: `lib/reporter.js` is a stateless client (zero credentials, zero `fs`) that POSTs anonymized crash/hang reports to a Cloudflare Worker at `workers/crash-reporter/index.ts` (with `workers/crash-reporter/wrangler.toml`), which holds the GitHub App credentials as secrets and files GitHub Issues. `AGENTS.md` "Telemetry" section describes this pipeline and the `tests/unit/noSecrets.test.js` guard.
- **Source path:** `sources/camofox-browser/lib/browser-errors.js`, `sources/camofox-browser/lib/browser-processes.js`, `sources/camofox-browser/lib/new-page-recovery.js`, `sources/camofox-browser/lib/process-ownership.js`, `sources/camofox-browser/workers/crash-reporter/`, `sources/camofox-browser/AGENTS.md`
