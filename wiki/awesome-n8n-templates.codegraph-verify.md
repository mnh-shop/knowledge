...
- **Source evidence:**
  - The repo contains 19 template directories containing `.json` workflow files (plus `docs/` and `img/` for non-template content).
  - `OpenAI_and_LLMs/` alone contains 90 workflow JSON files.
  - `Gmail_and_Email_Automation/` contains 26 email workflow JSON files.
  - `Google_Drive_and_Google_Sheets/` contains 15+ workflow files.
  - `Instagram_Twitter_Social_Media/` contains 14 social media workflow files.
  - `Telegram/` contains 25 Telegram bot workflows.
  - `Slack/` contains 9 Slack integration workflows.
  - `WhatsApp/` contains 5 WhatsApp chatbot workflows.
  - `WordPress/` contains 6 WordPress automation workflows.
  - Plus categories for Airtable, Notion, Discord, PDF processing, Database/Storage, DevOps, Forms/Surveys, AI Research, HR/Recruitment, Other Integrations — totaling 280+ imports (330 `.json` files on disk).
  - `README.md` states: "280+ free, ready-to-import workflow templates" and links to import instructions.
- **Verdict:** ✅ CORRECT
- **Fix needed:** None

## Claim 2: Categories organized by platform/integration (19 template dirs)
...
- **Source evidence:**
  - Repository directory structure has 19 template directories:
    ...
    - `WhatsApp/`, `WordPress/`, `devops/`
  - `docs/` and `img/` are documentation/asset directories, NOT template categories (excluded from the 19-count).
  - `README.md` claims "280+ automation templates across 18 categories" (README line 94) — the README's 18-count is a source-side undercount vs the 19 template dirs on disk (incl. `Other/`, `Other_Integrations_and_Use_Cases/`, `devops/`).
...
- **Verdict:** ✅ CORRECT
- **Fix needed:** None (verify page previously overcounted 21 dirs by including `docs/` and `img/`)

## Claim 3: Multi-language README (13 translations + English = 14 READMEs)
- **Wiki says:** The README is translated into 13 languages plus English — Turkish, Chinese, German, French, Spanish, Portuguese, Japanese, Korean, Hindi, Arabic, Indonesian, Russian, and Italian (14 READMEs total).
- **Source evidence:**
...
- **Verdict:** ✅ CORRECT (13 translations + English = 14 READMEs; Portuguese included)
- **Fix needed:** None
...
- **Source evidence:**
  - `OpenAI_and_LLMs/` is the largest category with 90 workflow files covering:
...
## Claim 7: CC BY 4.0 license with CI/CD and contribution guidelines
- **Wiki says:** The repository is licensed under CC BY 4.0 and includes CI/CD via GitHub Actions, contribution guidelines, and a citation file for academic reference.
- **Source evidence:**
  - `README.md` badge: `license-CC--BY--4.0-blue` — Creative Commons Attribution 4.0 International (CC-BY-4.0).
  - `CONTRIBUTING.md` (2301 bytes) — contribution guidelines.
  - `CITATION.cff` (745 bytes) — citation metadata.
  - `.github/workflows/pr-review.yml` — GitHub Actions CI/CD configuration.
- **Verdict:** ✅ CORRECT
- **Fix needed:** None (wiki already states CC BY 4.0 — earlier license-correction note is obsolete)
...
- ✅ 280+ workflow templates confirmed across 19 template directories (330 `.json` files on disk)
- ✅ Platform-organized categories confirmed with descriptive navigation
- ✅ 13 translated READMEs + English (14 READMEs) confirmed at repository root, incl. Portuguese
- ✅ Quick-start documentation with 5-step import instructions confirmed
- ✅ AI/LLM template focus confirmed (90 workflows in OpenAI_and_LLMs alone)
- ✅ llms.txt for AI agent consumption confirmed
- ✅ CC BY 4.0 license confirmed (matches wiki; no correction needed)
