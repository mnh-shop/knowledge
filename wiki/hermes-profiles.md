...
- **Shared skill pool with symlink-based reuse** — the root `skills/` directory contains 54 skill categories (architecture ADRs, artifact-pyramids, backend-engineering, research-methodology, mermaid-diagrams, financial-modeling, go-to-market, security-audit-methodology, etc.) as the single canonical copy. Each profile's `skills/` directory contains only relative symlinks back to the shared pool (`../../../skills/<name>`, git mode 120000). Git tracks symlinks by reference, so clones reproduce correctly on macOS and Linux without duplicating content. This means one copy of each skill serves every profile, and adding a new skill to the pool immediately makes it available to all profiles.
...
- **CI/CD-ready contribution workflow** — profiles are contributed via GitHub PRs (`feat/your-profile` branch) with a clear checklist: SOUL.md with first principles and output contract, profile.yaml with valid YAML and required skills, README.md with installation and quick-start, AGENTS.md with trigger patterns, resolved relative symlinks, and no duplicate skill files. The `validate_profiles.py` script can be run as a CI gate (`profile-checks.yml`).

- **Factory Droid automation (CI/CD)** — two additional GitHub Actions workflows drive AI-assisted review and automation:
  - `droid.yml` — "Droid Tag" automation triggered on issue/PR creation, review submissions, and any comment containing `@droid`; runs `Factory-AI/droid-action` (gated to `github.repository_owner` actions only).
  - `droid-review.yml` — "Droid Auto Review" on every opened/ready/reopened PR with **depth assessment**: the changed-file list is matched against security-sensitive path patterns (`(auth|oauth|jwt|saml|password|api.?key|secret|token|credential|crypt|cert|ssl|tls|firewall|proxy|dockerfile|docker-compose|kubernetes|k8s|terraform|\.env|config\.|sudo|exec|eval|spawn|cve|exploit)`) — deep review + `security=true` on a match, shallow otherwise.

## Repository Structure

```
hermes-profiles/
├── skills/                          ← Shared skill pool (actual files, 54 categories)
...
├── .github/workflows/
│   ├── profile-checks.yml           ← CI gate: validate + README count check
│   ├── droid.yml                    ← @droid automation
│   └── droid-review.yml             ← Depth-assessed AI review
├── .hermes/plans/                   ← (unexplained in repo) plan files, e.g. 2026-06-05_123300-platform-engineer-profile.md
├── CONTRIBUTING.md
├── AGENTS.md
└── README.md
```
...
## Profile → Skills Mapping

Every one of the 39 profiles declares its skill dependencies in `profile.yaml` under `skills.required` (and optionally `skills.recommended`). **All 39 profiles list `artifact-pyramids` as their first required skill** — the shared output-contract skill — then a role-specific methodology skill. Example rows:

| Profile | Required skills | Recommended skills |
|---------|-----------------|--------------------|
| researcher | artifact-pyramids, research-methodology, researcher-workflow | — |
| technical-architect | artifact-pyramids, c4-diagramming, adr-authoring, arc42-context, architect-pyramid | — |
| backend-engineer | artifact-pyramids, backend-engineering | — |
| ceo | artifact-pyramids, executive-methodology, strategy-frameworks | mermaid-diagrams |
| cto | artifact-pyramids, executive-methodology, technology-radar, architecture | architecture, mermaid-diagrams |
| cfo | artifact-pyramids, executive-methodology, financial-modeling | — |
| platform-engineer | artifact-pyramids, platform-engineering, docker-management, traefik | traefik, tailscale, implementation-planning, mermaid-diagrams |
| spec-driven-development | artifact-pyramids, sdd-authoring, sdd-work-decomposition, sdd-verification, sdd-review | — |
| security-engineer | artifact-pyramids, security-audit-methodology | — |

Patterns: the 9 C-suite profiles all require `executive-methodology` on top of `artifact-pyramids`; engineering profiles pair a single role methodology (e.g. `frontend-engineering`, `ml-engineering`, `data-engineering`); recommended skills are used for cross-cutting extras (most often `mermaid-diagrams`).
