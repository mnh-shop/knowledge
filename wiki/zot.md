...
- **SKILL.md support** — File-based tool definitions for collaborative workflows, discovered at startup and hot-reloadable
- **Sandbox path-jailing** — `/jail` command confines read/write/edit/bash to the session cwd; blocks `sudo`, `rm -rf /`, `cd /`, `cd ..`, and similar escapes
- **Model auto-discovery** — Background refresh from provider `/v1/models` endpoints, cached for 6h in `$ZOT_HOME/models-cache.json`
- **Custom models/providers** — `models.json` in `$ZOT_HOME` for user-defined models with custom `baseUrl` and API wire format (OpenAI or Anthropic)
