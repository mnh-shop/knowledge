...
| [[oh-my-pi-api|oh-my-pi API]] | oh-my-pi | CLI-only tool — no REST API |
| [[openclaw-api|OpenClaw API]] | openclaw | WebSocket RPC + OpenAI-compatible HTTP endpoints + MCP/ACP surfaces |
| [[pi-api|Pi API]] | pi | CLI-only tool — no REST API |
...
| [[zot-api|Zot API]] | zot | Telegram polling bot, JSON-RPC stdin/stdout, Go SDK embedding, no REST HTTP server |
